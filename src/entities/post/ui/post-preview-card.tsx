...
export function PostPreviewCard(props: PostPreviewCardProps) {
  ...
  return (
    <article
      class="group relative px-5 py-5 transition duration-200 hover:bg-surface-subtle sm:px-6"
      data-thread-grouped={isThreadGrouped() ? "true" : "false"}
      data-thread-position={threadPosition()}
    >
      {shareSummary() && (
        <p class="mb-3 pl-14 text-xs font-medium text-fg-muted" data-share-summary="true">
          {shareSummary()}
        </p>
      )}
      <div class="flex gap-x-3">
        <div class="relative flex w-11 shrink-0 justify-center items-start self-stretch">
          <span
            class="relative z-10 inline-flex shrink-0 rounded-full bg-surface-glass p-0.5"
          ...
          <div class="flex flex-wrap items-center gap-x-3 gap-y-2 text-sm text-fg-secondary">
            <div class="flex min-w-0 items-center gap-3">
              <div class="flex min-w-0 flex-col flex-wrap gap-y-1">
                ...
                <span class="font-code text-sm text-fg-muted">{props.post.author.handle}</span>
              </div>
            </div>
          </div>
          <div>
            {isArticle() ? (
              <>
                <a
                  class="mt-3 block text-[1.45rem] font-semibold leading-tight tracking-[-0.03em] text-fg-primary transition group-hover:text-accent-strong"
                  href={props.post.href}
                >
                  {props.post.title}
                </a>
                <p class="mt-3 max-w-3xl text-sm leading-7 text-fg-secondary">
                  {props.post.excerpt}
                </p>
              </>
            ) : (
              <a
                class="mt-3 block max-w-3xl text-base leading-8 text-fg-primary transition group-hover:text-accent-strong"
                href={props.post.href}
              >
                {props.post.excerpt}
              </a>
            )}
            <ul class="mt-4 flex flex-wrap gap-4 text-sm font-medium tracking-[0.18em] text-fg-muted">
